...
// === 출근 ===
const commute = await getFastestRouteAction({ query: "출근" }, route);
...
// === 퇴근 ===
const goHome = await getFastestRouteAction({ query: "퇴근" }, route);
...
// === 퇴근 낭만 ===
const romantic = await getGoodRoute({ query: "퇴근 낭만" }, route, place);
...
// === 회사에서 홍대입구 빨리 ===
const toHongdae = await getFastestRouteAction({ query: "회사에서 홍대입구 빨리" }, route);
...
// === 강남역 빨리 — 출발지 방어 ===
const noOrigin = await getFastestRouteAction({ query: "강남역 빨리" }, route);
assert(noOrigin.includes("출발지"), "출발지방어: 출발지 질문");
assert(noOrigin.includes("알려주세요"), "출발지방어: 질문 톤");

// === 비 오는데 퇴근 — 날씨 context ===
const rainCommute = await getFastestRouteAction({ query: "비 오는데 퇴근" }, route);
...
const rainGood = await getGoodRoute({ query: "비 오는데 퇴근 낭만" }, route, place);
...
const lateAppt = await getFastestRouteAction({ query: "약속 늦었어", currentLocation: "강남역 근처" }, route);
...
// === 약속 늦었어 — 목적지 없이 ===
// Reset profile to clear recentDestination
await saveUserPlaces({ home: "신림역 근처", work: "강남역 근처" });
const lateNoTarget = await getFastestRouteAction({ query: "약속 늦었어" }, route);
assert(lateNoTarget.includes("알려주세요"), "늦었어(모름): 목적지 질문");

// === 산책길 ===
await saveUserPlaces({ home: "신림역 근처", work: "강남역 근처" });
const walk = await getGoodRoute({ query: "퇴근 20분만 돌아가는 산책길", placeType: "walk" }, route, place);
assert(walk.includes("산책"), "산책: 산책 키워드");
assert(walk.includes("걸어"), "산책: 걸어가세요");

// === 혼밥 ===
const honbap = await getGoodRoute({ query: "회사에서 홍대입구 약속 전에 혼밥", placeType: "restaurant" }, route, place);
assert(honbap.includes("덮밥"), "혼밥: 덮밥 추천");
assert(honbap.includes("홍대"), "혼밥: 홍대 목적지");

// === 집 가는 길 카페 ===
const homeCafe = await getGoodRoute({ query: "집 가는 길 카페", currentLocation: "강남역 근처" }, route, place);
assert(homeCafe.includes("카페"), "집카페: 카페 포함");
assert(homeCafe.includes("신림"), "집카페: 신림 방향");

// === Fixture diversity ===
const gangnam2Sinrim = await getFastestRouteAction({ query: "퇴근" }, route);
assert(gangnam2Sinrim.includes("146번"), "fixture: 강남→신림 = 146번");

await saveUserPlaces({ home: "신림역 근처", work: "강남역 근처" });
const sinrim2Gangnam = await getFastestRouteAction({ query: "출근" }, route);
