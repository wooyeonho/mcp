...
export function createMcpServer(userId: string) {
  ...
  mcp.tool(
    ...
    fastestSchema,
    async (input) => text(await getFastestRouteAction(input, routeProvider, userId)),
  );
  mcp.tool(
    "get_good_route",
    "기분 좋은 경로를 추천합니다. '퇴근 낭만', '카페 들렀다 가자', '산책', '혼밥', '약속 전에 밥' 같은 여유 있는 이동에 사용하세요.",
    goodRouteSchema,
    async (input) => text(await getGoodRoute(input, routeProvider, placeProvider, userId)),
  );
  return mcp;
}
    ...
    try {
      ...
      if (!transport && isInitializeRequest(req.body)) {
        ...
        transport = new StreamableHTTPServerTransport({
          sessionIdGenerator: () => sessionUserId,
