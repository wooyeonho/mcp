...
export class MockRouteProvider implements RouteProvider {
  async getRouteOptions(req: RouteRequest): Promise<RouteOption[]> {
    const subway: RouteOption = { mode: "subway", durationMinutes: 38, line: "2호선", direction: `${req.destination} 방면`, boardAt: req.origin.includes("강남") ? "강남역" : `${req.origin} 인근역`, alightAt: req.destination.includes("신림") ? "신림역" : `${req.destination} 인근역`, firstWalkMinutes: 5, lastWalkMinutes: 6, fareKrw: 1550, missedFallback: "이번 열차를 놓치면 다음 열차 기준 약 4~6분 더 걸립니다." };
    const bus: RouteOption = { mode: "bus", durationMinutes: 47, busNumber: "146번", boardAt: `${req.origin} 정류장`, alightAt: `${req.destination} 정류장`, firstWalkMinutes: 4, lastWalkMinutes: 7, fareKrw: 1500, missedFallback: "146번을 놓치면 같은 정류장에서 다음 버스를 기다리세요." };
    const taxi: RouteOption = { mode: "taxi", durationMinutes: 32, boardAt: req.origin, alightAt: req.destination, firstWalkMinutes: 1, lastWalkMinutes: 1, taxiFareKrw: 18000, missedFallback: "택시가 바로 안 잡히면 지하철로 전환하세요." };
